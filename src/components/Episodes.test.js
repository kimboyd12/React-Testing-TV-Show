...
test('re-renders correctly with the list of episodes', () => {
    // arrange component & set up mock data
    const {rerender, queryAllByTestId, getAllByTestId} = render(<Episodes episodes= {[]} />)
    // check that no episodes when component first renders
    ...
    // Act - rerender the component with the episodes data
    rerender(<Episodes error='' episodes={data} />);
    const episodes = getAllByTestId(/episode/i);

    // Assert - making sure the episodes are rendered to the page
    // mock data has 3 episodes in it so it should render 3
